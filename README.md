# Hi, I'm Michael Abbott 👋

I am a **Professor of Theater** and **Chair of the Music Department** at **Wabash College**. My work lives at the intersection of performance, pedagogy, and digital storytelling, with a particular focus on how narrative functions across different media.

---

### 🎓 Academic & Creative Focus

* **Interactive Narrative:** Researching the evolution of storytelling and player agency within Role-Playing Games (RPGs).
* **Film & Media:** Analyzing the impact of film scores and global cinema trends.
* **Theater & Performance:** Writing and developing new works for the stage and exploring the future of the arts in higher education.

### 🔭 Currently Working On

* **World Cinema 2026:** Developing a comprehensive curriculum on global film history and theory.
* **RPG Seminar:** Designing a college-level course focused on the narrative structures of modern video games.
* **Digital Tools:** Experimenting with AI integration in the classroom and audio production via Audacity and RapidComposer.

### 🎙️ Beyond the Classroom

I have a long-standing history in the gaming community, formerly hosting **The Brainy Gamer** podcast. When I’m not in a rehearsal hall or a lecture hall, you can usually find me at a baseball game, traveling, or exploring the cultural significance of play.

---

### 📫 Connect with me

* **Music Portfolio:** [[LittleBigMusical.com](http://LITTLEBIGMUSICAL.COM](https://soundcloud.com/michaelabbott-723532162))
* **Institutional Profile:** [[Wabash College Faculty Page](https://www.wabash.edu/aboutwabash/directories/staff/list?search=michael+abbott&dept=)]
* **Gaming:** [Steam/PSN Links]

> "The arts are not a luxury, but a necessity for a well-rounded life."

---

### How to set this up:

1. **Create a Repository:** Name it exactly your GitHub username (e.g., `mabbott` or `wabashprof`).
2. **Add File:** Create a file named `README.md`.
3. **Paste & Save:** Paste the code above and click **Commit changes**.

**Would you like me to add a specific section for your current research on retirement options for expats, or perhaps a list of your latest theatrical plays?**## Hi there 👋

<!--
**brainygamer/brainygamer** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
